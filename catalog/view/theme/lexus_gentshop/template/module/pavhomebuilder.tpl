...
<?php $rows = $layouts; $level = 1; } ?>	
 	...
 	?>
	    <?php if ( $row->level==1 ){	 ?>
	        <div class="container<?php if ($row->fullwidth==1 ) { ?>-fluid<?php } ?> <?php if ( isset($row->parallax)&&$row->parallax ) { ?> pts-parallax <?php } ?>" >
	        	<div class="pav-inner <?php echo $row->cls; ?>" <?php echo $row->attrs?>>
	    <?php } ?>  
