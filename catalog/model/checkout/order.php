...
class ModelCheckoutOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'purchase_order_no'		  => $order_query->row['purchase_order_no'],
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $notify_admin = true) {
		...
		if ($order_info) {
			...
			//if (!$order_info['order_status_id'] && $order_status_id) {
			  ...
			  if ($order_status_id) {
				...
				$subject = sprintf($language->get('text_new_subject'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'), $order_id, $order_status);
                ...
                //$data['reg_no'] = $order_info['custom_field'][6];

                $data['reg_no'] = $order_info['purchase_order_no'];
				...
				if ($order_info['payment_address_format']) {
					...
					$format = '{firstname} {lastname}' . "<br>" . '{company}' . "<br>" . '{address_1}' . "<br>" . '{address_2}' . "<br>" . '{city} {postcode}' . "<br>" . '{zone}' . "<br>" . '{country}';
				...
				$data['payment_address'] = str_replace(array("\r<br>", "\r", "<br>"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/<br><br>+/"), '<br />', trim(str_replace($find, $replace, $format))));
				...
				$data['shipping_address'] = str_replace(array("\r<br>", "\r", "<br>"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/<br><br>+/"), '<br />', trim(str_replace($find, $replace, $format))));
				...
				if (!$this->model_payment_amazon_checkout->isAmazonOrder($order_info['order_id'])) {
					...
					$text  = sprintf($language->get('text_new_greeting'), html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8')) . "<br><br>";
					...
					if ($comment && $notify) {
						...
						$text .= $language->get('text_new_instruction') . "<br><br>";
					...
					if ($order_info['customer_id']) {
						...
						$text .= $language->get('text_new_link') . "<br>";
					...
					if ($download_status) {
						...
						$text .= $language->get('text_new_download') . "<br>";
					...
					$text .= $language->get('text_new_footer') . "<br><br>";
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				if ($order_status_query->num_rows) {
					...
					$message .= $language->get('text_update_order_status') . "<br><br><b>";
				...
				if ($comment) {
					...
					$message .= $language->get('text_update_comment') . "<em><br><br>";

					$message .= strip_tags($comment) . "</em><br><br>If you have any questions or need assistance please contact us: <a href='mailto:support@thenos.us'>support@thenos.us</a>
...
";
                ...
                $alternate_emails = explode(',', $order_info['custom_field'][4]);



				foreach ($alternate_emails as $alternate_email) {

					if ($alternate_email && preg_match('/^[^\@]+@.*.[a-z]{2,15}$/i', $alternate_email)) {

						$mail->setTo($alternate_email);
